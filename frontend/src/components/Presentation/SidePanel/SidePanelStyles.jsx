...
import type {Props, BgProps} from "./SidePanelTypes.jsx";
...
const getStyles: GetStyles<Props, Styles> = props => {
  ...
  return {
    ...
    contentWrapper: {
      ...
      left,
      padding:
        "calc(" + headerHeightCoef + " * " + appStyles.unitHeight + ") 0 0  0",
...
export const getBgStyles: GetStyles<BgProps, BgStyles> = props => {
  let panelWidth =
    "calc(" + props.widthCoef + " * " + appStyles.unitWidth + ")";
  const {clientWidth} = document.documentElement;

  let opacity =
    props.isFooterOpened && props.type === sidePanelTypes.DEFAULT ? 0 : 1;
