...
const mapDispatchToProps: MapDispatchToProps<ReduxDispatch> = function(
  ...
  return {
    updateIsSidePanelOpened: isOpened =>
      dispatch(updateIsSidePanelOpened(isOpened)),
  };
};
...
const BG = (props: BgProps) => {
  ...
  const widthCoef =
    clientWidth < appStyles.mediaQueries.phone
      ? clientWidth / appStyles.unitWidthJs
      : props.widthCoef;
  ...
  let coord2Circ =
    props.type === sidePanelTypes.DEFAULT
      ? coord2CircDefault
      : coord2CircInverted;
  ...
  if (props.type === sidePanelTypes.INVERTED) {
    ...
    p2 = {
      x: 0,
      y: coord2Circ(clientWidth - widthCoef * appStyles.unitWidthJs).y1,
    };
  ...
  return (
    <div style={styles.wrapper}>
      <svg
        ...
        <path
          d={path}
          style={{fill: color, transition: "all 0.5s linear"}}
          stroke="rgba(0, 0, 0, 0)"
          strokeWidth="3"
        />
      </svg>
...
const getToggleButtonStyles = props => {
  ...
  return {
    wrapper: {
      ...
      top: y,
      transition:
        "transform " + appStyles.sidePanel.transitionTime / 500 + "s linear",
      transform: props.isOpened
        ? "translateY(-50%)"
...
const ToggleButton: React.StatelessFunctionalComponent<
  ToggleButtonProps,
> = props => {
  ...
  componentDidUpdate: function(prevProps: Props) {
    if (this.props.isOpened !== prevProps.isOpened) {
      ...
      requestAnimationFrame(() =>
        togglePanel(
          prevProps.isOpened ? openedWidthCoef : 0,
          prevProps.isOpened ? 0 : openedWidthCoef,
          0,
          this.props.setWidth,
          new Date().getTime(),
        ),
...
export {sidePanelTypes, BG, sidePanelLifecycle, SidePanel, SidePanelSmart};
