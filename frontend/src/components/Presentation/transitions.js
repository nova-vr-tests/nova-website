...
const updateBackgroundLayers = (sign, pages, currentPage) => {
  ...
  const backLayers =
    sign < 0 ? pages[previousPage].layers : currentlyShownLayers;
  const frontLayersPid =
    sign < 0 ? pages[currentPage].pid : pages[nextPage].pid;
  const backLayersPid =
    sign < 0 ? pages[previousPage].pid : pages[currentPage].pid;

  const frontLayers2 =
    sign > 0
      ? transitions.splitBackground.resetBackgroundStyles(frontLayers, 0)
      : transitions.splitBackground.resetBackgroundStyles(frontLayers, 1);
  const backLayers2 =
    sign > 0
      ? transitions.splitBackground.resetBackgroundStyles(backLayers, 0)
      : transitions.splitBackground.resetBackgroundStyles(backLayers, 1);

  const isBackLayerVisible =
    store.getState().bgReducer.transitionProgress < 0.9 ? true : false;
  ...
  if (sign > 0) {
    if (isBackLayerVisible) {
      ...
      dispatch(
        updateCacheLayers(
          resetBackgroundStyles(frontLayers, 1),
          frontLayersPid,
        ),
      );
    ...
    if (!isBackLayerVisible) {
      ...
      dispatch(
        updateCacheLayers(
          resetBackgroundStyles(currentSlideLayers, 1),
          currentSlideLayersPid,
        ),
      );
      ...
      dispatch(
        updateCacheLayers(
          updateLayersOpacity(currentSlideLayers, 1),
          currentSlideLayersPid,
        ),
      );
      dispatch(
      ...
      dispatch(
        updateBackLayers(
          updateLayersOpacity(previousSlideLayers, 0),
          previousSlideLayersPid,
        ),
      );
    }
  }
  ...
  window.setTimeout(() => {
    ...
    if (sign < 0) {
      dispatch(
        updateBackLayers(
          updateLayersOpacity(previousSlideLayers, 1),
          previousSlideLayersPid,
        ),
      );
    }
...
const updateLayersOpacity: UpdateLayersOpacity = (layers, opacity) =>
  layers.map(e => ({
    ...e,
...
const updateFrontLayersOpacity: UpdateFronLayersOpacity = (
  layers,
  progress,
  layersPid,
) => {
...
const slideTransition = (
  ...
  const {
    frontLayers,
    frontLayersPid,
  } = transitions.splitBackground.updateBackgroundLayers(
    sign,
    pages,
    currentPage,
  );
  ...
  if (
    (sign < 0 && currentPage <= 0) ||
    (sign > 0 && currentPage >= totalPages - 1)
  ) {
  ...
  transitionTimer = window.setInterval(() => {
    if (transitionProgress > 100) {
      ...
      if (sign > 0) {
        updateFrontLayersOpacity(
          frontLayers,
          (transitionProgress / 100) * 2,
          frontLayersPid,
        );

        dispatch(
          updateTransitionProgress(
            transitionProgress / 100 < 0.5
              ? 0
              : (transitionProgress / 100 - 0.5) * 2,
          ),
        );
      } else {
        updateFrontLayersOpacity(
          ...
          transitionProgress < 50
            ? 1
            : 1 - (transitionProgress / 100 - 0.5) * 2,
          frontLayersPid,
        );

        dispatch(
          updateTransitionProgress(
            1 - transitionProgress / 100 < 0.5
              ? 0
              : (1 - transitionProgress / 100 - 0.5) * 2,
          ),
        );
...
*/
const bgParalaxSlideTransition = (
  sign,
  pages,
  currentPage,
  attachScrollEvent,
  ...
  let updateLayers =
    bgState.transitionProgress !== 0 ? updateFrontBg : updateBackBg;
...
const startTransition = (type, params) => {
  ...
  switch (type) {
    case BG_SPLIT:
      console.log(
        transitions.splitBackground.slideTransition(
          sign,
          pages,
          currentPage,
          attachScrollEvent,
    ...
    case BG_PARALAX:
      console.log(
        transitions.bgParalax.slideTransition(
          sign,
          pages,
          currentPage,
          attachScrollEvent,
