...
const BlogPostContent = props => {
  ...
  const P = props => {
    if (props.children[0].length > 4) {
      if (props.children[0].substring(0, 4) === "!!! ") {
        ...
        return (
          <p style={importantPStyles}>
            {props.children[0].substring(4, props.children[0].length)}
          </p>
        );
      }
    ...
    return <p style={styles.p}>{props.children}</p>;
  };
  ...
  const renderers = {
    ...
    table: props => (
      <div style={styles.tableWrapper}>
        <table className="zui-table zui-table-rounded" style={styles.table}>
          {props.children}
        </table>
      </div>
    ),
  ...
  return <ReactMarkdown renderers={renderers} source={props.content} />;
};
