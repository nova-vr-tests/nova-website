...
import getStyles from "./BlogStyles.jsx";
...
const ContentWrapper = props => {
  ...
  return <div style={styles.articleWrapper}>{props.children}</div>;
};
...
const Blog = props => {
  ...
  const squarePictoUrl = props.blogPost.squarePicto
    ? new URL(props.blogPost.squarePicto)
    : {origin: "", pathname: ""};
  const pictoUrl = props.blogPost.picto
    ? new URL(props.blogPost.picto)
    : {origin: "", pathname: ""};
  const headerPictoUrl =
    squarePictoUrl.origin + squarePictoUrl.pathname === ""
      ? pictoUrl
      : squarePictoUrl;

  return (
    <div style={styles.wrapper} className="Blog--wrapper">
      <div style={styles.headerWrapper}>
        <Header
          pictoUrl={
            props.auth
              ? pictoUrl.origin + pictoUrl.pathname
              : headerPictoUrl.origin + headerPictoUrl.pathname
          }
          title={title}
        />
      </div>
...
const fetchBlogPost = async (fetchUrl, setBlogPost, that) => {
  ...
  let blogPost;
  if (that.props.auth) {
    blogPost = await restApi.fetchDetailAuth(
      fetchUrl,
      postId,
      that.props.password,
    );
  ...
  if (that.mounted) {
    ...
    if (window.location.pathname !== "/products") {
      ...
      const imgUrl =
        ...
        that.props.pages[that.props.currentPage].layers[0].imgUrl;
...
const SmartComp = compose(
  withState("blogPost", "setBlogPost", {}),
  lifecycle({
    componentDidMount() {
      this.mounted = true;
