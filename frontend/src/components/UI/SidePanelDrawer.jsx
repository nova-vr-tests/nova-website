...
import getStyles from "./SidePanelDrawerStyles.jsx";
...
const SidePanelDrawer = props => {
  ...
  return (
    <div style={styles.mainWrapper} className="SidePanelDrawer--wrapper">
      <div style={styles.wrapper}>{props.childComps}</div>
    </div>
  );
};
...
const initChildComps = props => {
  ...
  for (let i in props.comps) {
    ...
    tmp.push(
      <div key={i} style={styles.centerWrapper}>
        <SmartComp />
      </div>,
    );
  }
...
const updateChildComps = (props, nextProps) => {
  ...
  for (let i in props.comps) {
    ...
    if (props.comps[i] !== nextProps.comps[i]) {
      ...
      const Cc = connect(reduxState)(() => (
        <div style={getStyles(props).centerWrapper}>
          <SmartComp key={i} />
        </div>
      ));
      tmp[i] = <Cc key={i} />;
...
const SmartComp = compose(
  withState("childComps", "setChildComps", "ss"),
  lifecycle({
    componentDidMount() {
...
export {SmartConnectedComp as SmartSidePanelDrawer};
