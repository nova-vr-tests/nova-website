...
import type {Props, PresentationControlsProps} from "./FooterTypes.jsx";
...
const getStyles: GetStyles<Props, Styles> = props => {
  ...
  let footerOffset =
    props.isFooterOpened && isIntroFinished
      ? "(100vh - " +
        (footerHeight + footerRadiusOffset + props.sidebarHeaderIntersection) +
        "px)"
      : "0vh";
  ...
  return {
    ...
    svg: {
      transform:
        "rotateX(180deg)translateY(calc(0.5 * " + appStyles.unitHeight + "))",
      position: "absolute",
      top: "0",
      width: "100vw",
      pointerEvents: "none",
    ...
    wrapper: {
      transition:
        props.introKeyframe >= INTRO_FINISHED
          ? "background-color " +
    ...
    toggleSidebarButton: {
      ...
      transition:
        "transform " +
        appStyles.sidebar.hoverTransition.length +
        ", opacity " +
        appStyles.sidebar.hoverTransition.length +
        appStyles.sidebar.hoverTransition.type,
      bottom: "calc(" + appStyles.unitHeight + " / 3)",
      left:
        "calc(" +
        appStyles.sidebar.widthFactor +
        " / 2 * " +
        appStyles.unitWidth +
        " - " +
        height +
        ...
        " / 3)",
      transform:
        "rotateZ(45deg)translateX(calc(0.99 / 3 * " +
        width +
        "))translateY(calc(-0.50 / 3 * " +
        height +
        "))",
      cursor: "pointer",
    ...
    quickLinks: {
      ...
      cursor: "pointer",
      opacity:
        props.introKeyframe >= INTRO_FINISHED + 1 && !props.isFooterOpened
          ? 1
          : 1,
    },
...
const getPresentationControlsStyles: GetStyles<
  PresentationControlsProps,
  PresentationControlsStyles,
> = props => {
  return {
    wrapper: {
      fontSize: "5rem",
...
export {getPresentationControlsStyles};
