...
const PageWrapper = (props: UIprops) => {
  ...
  return (
    <div style={{...styles.pageWrapper, ...{right}}}>{props.children}</div>
  );
};
...
const FlexColumn = props => {
  ...
  return (
    <div className={"FlexColumn--wrapper"} style={styles.wrapper}>
      {props.children}
    </div>
  );
};
...
const H1 = (props: UIprops) => {
  return <h1 style={{...styles.H1, ...props.style}}>{props.children}</h1>;
};
...
const SubmitButton = props => {
  const buttonStyle = {
    ...
    border: "none",
    color: "white",
    padding: `calc(0.25 * ${appStyles.unitWidth}) calc(1.5 * ${
      appStyles.unitWidth
    })`,
    margin: `calc(0.5 * ${appStyles.unitWidth}) calc(0.25 * ${
      appStyles.unitWidth
    })`,
    marginLeft: 0,
    cursor: "pointer",
